...
void replace_image_to_label(char *input_path, char *output_path) {
    ...
    find_replace1(input_path, "/images/train2014/", "/labels/train2014/", output_path);    // COCO
    find_replace1(output_path, "/images/val2014/", "/labels/val2014/", output_path);        // COCO
    find_replace1(output_path, "/JPEGImages/", "/labels/", output_path);    // PascalVOC
...
{   
    char txtname[] = "/home/spl6/hcz/yolov3/darknet_shuffle_test/test_6.txt"; 
       FILE *fp; 
       char StrLine[1024];             
   ...
   // srand(2222222);
    ...
    while(1){
        ...
        image ori_im = load_image_color(input,0,0);
    ...
    {   for (count2=0;count2<1;count2++)
      ...
      { image im = load_image_color(input,0,0);
       ...
       // printf("%s: Predicted in %f seconds.\n", input, what_time_is_it_now()-time);
        ...
        draw_detections(ori_im, dets, nboxes, thresh, names, alphabet, l.classes);
        if (1)
		{
			char labelpath[4096];
			...
			for (i = 0; i < nboxes; ++i) {
				...
				if (class_id >= 0) {
					sprintf(buff, "%d,%d,%d,%d,%f,%s\n", (int)((dets[i].bbox.x-dets[i].bbox.w/2)*im.w), (int)((dets[i].bbox.y-dets[i].bbox.h/2)*im.h),(int)((dets[i].bbox.w)*im.w), (int)((dets[i].bbox.h)*im.h),prob,"1");
        ...
        free_image(sized);}}
        if(outfile){
            save_image(ori_im, outfile);
        }
        /*else{
            save_image(ori_im, "predictions");
#ifdef OPENCV
            cvNamedWindow("predictions", CV_WINDOW_NORMAL); 
            if(fullscreen){
                cvSetWindowProperty("predictions", CV_WND_PROP_FULLSCREEN, CV_WINDOW_FULLSCREEN);
            }
            show_image(ori_im, "predictions", 0);
#endif
        }*/
...
/*void test_detector_label(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{   
   ...
   // srand(2222222);
    ...
    {   for (count2=0;count2<2;count2++)
        ...
        //count1=0,count2=0;
      { image im = load_image_color_re(input,0,0,count1,count2);
        image sized = letterbox_image(im, net->w, net->h);
        //image sized = resize_image(im, net->w, net->h);
        //image sized2 = resize_max(im, net->w);
        //image sized = crop_image(sized2, -((net->w - sized2.w)/2), -((net->h - sized2.h)/2), net->w, net->h);
       ...
       // printf("%s: Predicted in %f seconds.\n", input, what_time_is_it_now()-time);
        ...
        //if (nms) do_nms_obj(boxes, probs, l.w*l.h*l.n, l.classes, nms);
        if (nms) do_nms_sort(dets, nboxes, l.classes, nms);
        draw_detections2(ori_im, dets, nboxes, thresh, names, alphabet, l.classes,count1,count2);
        if (save_labels)
		{
			char labelpath[4096];
			replace_image_to_label(input, labelpath);
			...
			for (i = 0; i < nboxes; ++i) {
				...
				if (class_id >= 0) {
					sprintf(buff, "%d %2.4f %2.4f %2.4f %2.4f\r\n", class_id, dets[i].bbox.x+1024*count1, dets[i].bbox.y+1024*count2, dets[i].bbox.w, dets[i].bbox.h);
					fwrite(buff, sizeof(char), strlen(buff), fw);
				}
			}
			fclose(fw);
